...
class WavesEffect {
  constructor(container, options = {}) {
    ...
    this.config = {
      lineColor: options.lineColor || "rgba(226, 22, 22, 0.3)",
    ...
    this.lines.forEach((pts) => {
      pts.forEach((p) => {
        ...
        if (dist < l) {
          ...
          p.cursor.vx +=
            Math.cos(this.mouse.a) * f * l * this.mouse.vs * 0.00065;
          p.cursor.vy +=
            Math.sin(this.mouse.a) * f * l * this.mouse.vs * 0.00065;
        }
  ...
  drawLines() {
    ...
    this.ctx.strokeStyle = this.config.lineColor.replace(/[\d\.]+\)$/g, "1.0)"); // 提高透明度
    this.ctx.stroke();
  }
